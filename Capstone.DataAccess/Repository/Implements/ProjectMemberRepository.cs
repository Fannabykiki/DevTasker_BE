...
namespace Capstone.DataAccess.Repository.Implements
{
    public class ProjectMemberRepository : BaseRepository<ProjectMember>, IProjectMemberRepository
    {
		...
		{
            var projectMember = await _context.ProjectMembers.Where(x => x.ProjectId == projectId && x.Users.IsAdmin == false && x.StatusId == Guid.Parse("ba888147-c90a-4578-8ba6-63ba1756fac1")).Include(x => x.Users).Include(x => x.Role).Include(x=>x.Status).ToListAsync();
